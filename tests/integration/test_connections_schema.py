"""T037 — failing schema integration tests (must fail before T041–T042 impl)."""
import json
import pytest
import respx
import httpx
...
def test_schema_source_returns_fields(source_client):
    c, conn_id = source_client
    sample = '{"session_id":"sess-1","timestamp":"2026-01-01T00:00:00Z","cost":0.012}\n'
    with respx.mock:
        respx.get("http://ch:8123/").mock(return_value=httpx.Response(200, text=sample))
        r = c.get(
            f"/api/v1/connections/{conn_id}/schema",
...
def test_schema_sink_returns_400(sink_client):
...
def test_schema_sample_count_matches_records(source_client):
    c, conn_id = source_client
    sample = '{"session_id":"s1","cost":0.01}\n{"session_id":"s2","cost":0.02}\n'
    with respx.mock:
        respx.get("http://ch:8123/").mock(return_value=httpx.Response(200, text=sample))
        r = c.get(
            f"/api/v1/connections/{conn_id}/schema",
    ...
    body = r.json()
    # sample_count reflects the actual number of records sampled across all buckets
